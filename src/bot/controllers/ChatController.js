...
class ChatController {
    ...
    async handleMessage(msg) {
        try {
            ...
            const isMenuRequest = /^(menu|oi|olá|ola|oie|bom dia|boa tarde|boa noite|inicio|início|start|0)$/i.test(textL);
            ...
            const handledByBooking = await this.bookingController.handle(number, text, session, chat);
            ...
            if (session.step === "menu" || session.step === "start") {
                switch (text) {
                    ...
                    case "4":
                        await typing(chat);
                        await this.client.sendMessage(number, BotView.localizacaoEHorarios());
                        break;
                    default:
