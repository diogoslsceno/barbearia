...
class BookingController {
    ...
    async handleService(number, text, chat) {
        ...
        await this.client.sendMessage(number, BotView.erroOpcaoInvalida() + "\n\n" + BotView.menuServicos());
        return true;
    }
    ...
    async handleDate(number, text, chat) {
        ...
        if (dataRegex.test(text)) {
            ...
            if (!data.isValid() || data.isBefore(moment(), "day")) {
                await typing(chat, 1000);
                await this.client.sendMessage(number, "⚠️  Data inválida ou passada. Digite uma data futura no formato *DD/MM/YYYY*.");
                return true;
            }
            if (data.day() === 0) {
                await typing(chat, 1000);
                await this.client.sendMessage(number, "⚠️  Não abrimos aos domingos. Escolha outro dia. 😊");
                return true;
            }
        ...
        await typing(chat, 1000);
        await this.client.sendMessage(number, "⚠️  Formato inválido. Digite a data assim: *DD/MM/YYYY*\nEx: _25/06/2026_");
        return true;
    }
    ...
    async handleTime(number, text, chat) {
        ...
        if (!isNaN(idx) && horarios[idx]) {
            ...
            await typing(chat);
            await this.client.sendMessage(number, BotView.resumoAgendamento(SessionModel.get(number).data));
            return true;
        }
        await typing(chat, 1000);
    ...
    async handleConfirmation(number, text, chat) {
        ...
        if (text === "2") {
            SessionModel.reset(number);
            await typing(chat, 1000);
            await this.client.sendMessage(number, "❌ Agendamento cancelado.\n\n" + BotView.menuPrincipal());
            SessionModel.setStep(number, "menu");
            return true;
        }

        await typing(chat, 1000);
        await this.client.sendMessage(number, "Digite *1* para confirmar ou *2* para cancelar.");
        return true;
    }
}
